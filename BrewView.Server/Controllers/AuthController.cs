...
namespace BrewView.Server.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("user/{provider}")]
        public async Task<IActionResult> Get([FromRoute] string provider, [FromQuery] string codeChallenge, [FromQuery] string state)
        {
            return Redirect(await m_oAuthService.RedirectToAuthentication(Enum.Parse<AuthenticationProvider>(provider, true), codeChallenge, state));
        }

        [HttpGet]
        [Route("user/redirect/{provider}")]
        public async Task<IActionResult> GetTokens([FromRoute] string provider, [FromQuery] string code, [FromQuery] string codeVerifier)
        {
            var response = await m_oAuthService.RequestToken(code, Enum.Parse<AuthenticationProvider>(provider, true), codeVerifier);
